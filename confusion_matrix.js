function confusion_matrix(dataset) {
    console.log("I am in confusion_matrix.js")

    var svg = d3.select("#confusion-matrix").select("svg")

    var tsn = d3.transition().duration(1000);

    var margin = {top: 10, right: 85, bottom: 100, left: 150};
    ...
    // SVG canvas
    svg = d3.select("#confusion-matrix").select("svg")
    ...
    // Clip path
    svg.append("clipPath")
        .attr("id", "clipConfusionMatrix")
        .append("rect")
        .attr("width", width)
        .attr("height", height);

    svg.append('clipPath')
        .attr('id', 'xClipConfusionMatrix')
        .append("rect")
        .attr('x', 0)
        .attr('y', height)
        .attr("width", width)
    ...
    var heatDotsGroup = svg.append("g")
        .attr("clip-path", "url(#clipConfusionMatrix)")
        .append("g");
...
function renderConfusionMatrix(dataset) {
    var percentFormat = d3.format('.2');

    var svg = d3.select("#confusion-matrix")
        .select("svg");
    if (svg.select("#clipConfusionMatrix").empty()) {
        chartData = confusion_matrix(dataset);
    }
