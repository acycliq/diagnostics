function heatmap_2(dataset) {
    console.log("I am in heatmap.js")

    var svg = d3.select("#heat-chart_2").select("svg")
    ...
    // SVG canvas
    svg = d3.select("#heat-chart_2").select("svg")
    ...
    var heatDotsGroup = svg.append("g")
        .attr("clip-path", "url(#clipHeatMap_2)")
        .append("g");
...
function renderHeatmap_2(dataset) {
    var percentFormat = d3.format('.2');

    var svg = d3.select("#heat-chart_2")
        .select("svg");
    if (svg.select("#clipHeatMap_2").empty()) {
        chartData = heatmap_2(dataset);
    }
    ...
    update
        ...
        .on("mouseover", function (d) {
            $("#tooltip_heatmap").html("Cell class: " + d.xLabel + "<br/>Gene name: " + d.yLabel + "<br/>Avg counts: " + percentFormat(d.val));
